...
function [x, f] = optSolver(problem, method, options)
    ...
    while ((norm_g > options.term_tol * max(1, initial_norm)) && (k <= options.max_iterations))
        ...
        switch method.name
            ...
            case 'Newton'
                % take step according to a chosen method
                [x_new, f_new, g_new, H_new, ~, ~] = NewtonStep(x, f, g, H, problem, options);
                % update old and new function values
            ...
            case 'NewtonW'
                % take step according to a chosen method
                [x_new, f_new, g_new, H_new, ~, ~] = NewtonWStep(x, f, g, H, problem, options);
                % update old and new function values
                x = x_new; f = f_new; g = g_new; norm_g = norm(g, inf); H = H_new;
                % increment iteration counter
                k = k + 1;
            ...
            case 'TRSR1CG'
                [x_new, f_new, g_new, B_new, Delta_new, ~, ~] = TRSR1CGStep(x, f, g, B, term_tol_CG, max_iterations_CG, Delta, problem, options);
                x = x_new; f = f_new; g = g_new;
            ...
            case 'BFGS'
                ...
                if k == 0
                    ...
                    x = x_new; g = g_new;
                    [x_new, f_new, g_new, ~, ~] = BFGSStep(x, x_old, f, g, g_old, H_k, problem, options);
                    x_old = x; f = f_new; g_old = g; norm_g = norm(g, inf);
                    k = k + 1;
                end
            ...
            case 'BFGSW'
                if k == 0
                    ...
                    x = x_new; g = g_new;
                    [x_new, f_new, g_new, ~, ~] = BFGSWStep(x, x_old, f, g, g_old, H_k, problem, options);
                    x_old = x; f = f_new; g_old = g; norm_g = norm(g, inf);
                    k = k + 1;
                end
            case 'DFP'
                ...
                if k == 0
                    ...
                    x = x_new; g = g_new;
                    [x_new, f_new, g_new, ~, ~] = DFPStep(x, x_old, f, g, g_old, H_k, problem, options);
                    x_old = x; f = f_new; g_old = g; norm_g = norm(g, inf);
                    k = k + 1;
                end
            ...
            case 'DFPW'
                if k == 0
                    ...
                    x = x_new; g = g_new;
                    [x_new, f_new, g_new, ~, ~] = DFPWStep(x, x_old, f, g, g_old, H_k, problem, options);
                    x_old = x; f = f_new; g_old = g; norm_g = norm(g, inf);
                    k = k + 1;
                end
